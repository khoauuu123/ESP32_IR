...
void setup() {
  ...
  ac.next.protocol = decode_type_t::DAIKIN;       // Chọn giao thức điều khiển Daikin
...
void Turn_On_AC(int Temperature_Setting) {
  ...
  if (ac.isProtocolSupported(protocol)) {
    ...
    ac.next.fanspeed = stdAc::fanspeed_t::kMedium;  // Tốc độ quạt trung bình
    ac.next.swingv = stdAc::swingv_t::kHighest;     // Điều chỉnh quạt lên cao nhất
    ac.next.celsius = true;                         // Sử dụng đơn vị độ C
    ac.next.degrees = Temperature_Setting;          // Đặt nhiệt độ 26 độ C
    ac.next.power = true;                           // Bật điều hòa
    Serial.println("BẬT MÁY LẠNH: " + String(Temperature_Setting));
    ac.sendAc();  // Gửi tín hiệu điều khiển
    digitalWrite(LED, 1);
    delay(100);
    digitalWrite(LED, 0);
    delay(100);
